<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
    ...
    <h2 class="text-2xl font-bold text-gray-900 text-center mb-8">Stock Request</h2>

    <form id="stock-request-form" method="POST" action="{{ url('/marketing/transactions/request') }}">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            {{-- To Branch (locked to active) --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">To Branch</label>
                @if ($activeBranch)
                    <input type="hidden" name="to_branch_id" value="{{ $activeBranch->id }}">
        ...
        <button type="button" id="add-item-row"
            class="text-blue-600 text-sm hover:text-blue-700 mb-6">
            + Add Item
        </button>
...
<script>
(function () {
    // Item dataset for auto-fill on selection
    ...
    function rowHtml(idx) {
        ...
        return `
            <tr class="border-b border-gray-100 item-row">
                ...
                <td class="px-3 py-2">
                    <input type="text" readonly class="cat-cell w-full text-sm border border-gray-300 rounded px-2 py-1.5 bg-gray-50">
                </td>
                <td class="px-3 py-2">
                    <input type="text" readonly class="uom-cell w-full text-sm border border-gray-300 rounded px-2 py-1.5 bg-gray-50">
                </td>
                <td class="px-3 py-2">
                    <input type="number" min="1" step="1" value="1" name="items[${idx}][item_unit]"
                        class="qty-cell w-full text-sm border border-gray-300 rounded px-2 py-1.5 outline-none text-right" required>
                </td>
                <td class="px-3 py-2">
                    <input type="number" min="0" step="0.01" value="0" name="items[${idx}][unit_price]"
                        class="price-cell w-full text-sm border border-gray-300 rounded px-2 py-1.5 outline-none text-right">
                </td>
                <td class="px-3 py-2">
                    <input type="text" readonly value="0.00"
                        class="total-cell w-full text-sm border border-gray-300 rounded px-2 py-1.5 bg-gray-50 text-right">
                </td>
                <td class="px-3 py-2 text-center">
                    <button type="button" class="remove-row text-red-500 hover:text-red-700">&times;</button>
                </td>
            </tr>`;
    }
