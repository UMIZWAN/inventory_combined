...
@section('content')
    <div class="px-4">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="ml-2 text-xl font-semibold text-gray-800">Shipping Options</h3>
                    <p class="ml-2 mt-1 text-sm text-gray-500">Delivery methods used for transfers and transactions.</p>
                </div>
                @if ($canEdit)
                    <button id="add-shipping-trigger"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-4 py-3">
                <table id="shipping-table" class="w-full text-sm">
                    <thead>
                    ...
                    <tbody class="divide-y divide-gray-100">
                                ...
                                <td class="px-4 py-3">
                                    @if ($canEdit)
                                        <div class="flex items-center gap-2">
                                            <button
                                                class="edit-shipping-btn inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-emerald-700 bg-emerald-50 rounded-md hover:bg-emerald-100 transition-colors"
                                                data-id="{{ $option->id }}"
                                            ...
                                            <form method="POST" action="{{ url('/marketing/shipping-options/' . $option->id) }}"
                                                class="inline"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-4 py-10 text-center text-gray-400">No shipping options found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    {{-- Add/Edit Modal --}}
    <div id="shipping-modal" class="fixed inset-0 hidden flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 id="shipping-modal-title" class="text-xl font-bold text-gray-900">Add Shipping Option</h2>
                    </div>
                    <button id="close-shipping-modal" class="text-gray-400 hover:text-gray-600">
                ...
                <form id="shipping-form" method="POST" action="{{ url('/marketing/shipping-options') }}">
                    @csrf

                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Name <span class="text-red-500">*</span></label>
                            <input name="name" id="shipping-name" required
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                                placeholder="e.g. Bus, J&T Express, Staff Delivery">
                        </div>

                        <div>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="is_active" id="shipping-active" value="1" checked
    ...
    (function () {
        ...
        const openBtn = document.getElementById('add-shipping-trigger');
        const closeBtns = [
            document.getElementById('close-shipping-modal'),
            document.getElementById('cancel-shipping-modal'),
        ];
        ...
        if (openBtn) {
            openBtn.addEventListener('click', () => {
                form.reset();
                resetMethodSpoof();
                form.action = '/marketing/shipping-options';
                title.textContent = 'Add Shipping Option';
                submitBtn.textContent = 'Add Shipping Option';
                document.getElementById('shipping-active').checked = true;
        ...
        document.querySelectorAll('.edit-shipping-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                form.reset();
                resetMethodSpoof();
                form.action = `/marketing/shipping-options/${id}`;
                title.textContent = 'Edit Shipping Option';
                submitBtn.textContent = 'Update Shipping Option';

                document.getElementById('shipping-name').value = name;
                document.getElementById('shipping-active').checked = active;
