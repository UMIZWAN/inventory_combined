...
@section('content')
    <div class="px-4">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="ml-2 text-xl font-semibold text-gray-800">Categories</h3>
                    <p class="ml-2 mt-1 text-sm text-gray-500">Group marketing items by category.</p>
                </div>
                @if ($canEdit)
                    <button id="add-category-trigger"
            ...
            <div class="px-4 py-3">
                <table id="category-table" class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                                ...
                                <td class="px-4 py-3">
                                    @if ($canEdit)
                                        <div class="flex items-center gap-2">
                                            <button
                                                class="edit-category-btn inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-emerald-700 bg-emerald-50 rounded-md hover:bg-emerald-100 transition-colors"
                                            ...
                                            <form method="POST" action="{{ url('/marketing/categories/' . $category->id) }}"
                                                class="inline"
                                                onsubmit="return confirm('Delete this category? Items already assigned will block deletion.')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-4 py-10 text-center text-gray-400">No categories found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    {{-- Add/Edit Modal --}}
    <div id="category-modal" class="fixed inset-0 hidden flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 id="category-modal-title" class="text-xl font-bold text-gray-900">Add Category</h2>
                    </div>
                ...
                <form id="category-form" method="POST" action="{{ url('/marketing/categories') }}">
                    @csrf
                    <input type="hidden" id="method-spoof-slot">

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Name <span class="text-red-500">*</span></label>
                        <input name="name" id="category-name" required
                            ...
                            placeholder="e.g. Promotional Items">
                    </div>
    ...
    <style>
        ...
        .dataTables_wrapper .dataTables_length select {
            ...
            background-size: 1.25rem; appearance: none;
        }
    ...
    <script>
        ...
        const openBtn = document.getElementById('add-category-trigger');
        const closeBtns = [
        ...
        const methodSlot = document.getElementById('method-spoof-slot');

        function resetMethodSpoof() {
            // Remove any previously injected _method hidden field
        ...
        if (openBtn) {
            openBtn.addEventListener('click', () => {
                form.reset();
                resetMethodSpoof();
                form.action = '/marketing/categories';
        ...
        document.querySelectorAll('.edit-category-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                form.reset();
                resetMethodSpoof();
                form.action = `/marketing/categories/${id}`;
